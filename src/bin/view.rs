...
fn main() {
	...
	let post: Post = if let Some(row) = pool.first_exec(format!("SELECT title, content, post_time, edit_time, category FROM posts WHERE id = {}", id), ()).expect("Failed to get data from databas") {
