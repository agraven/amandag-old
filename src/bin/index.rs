...
const SELECT_COMMENT_COUNT: &'static str =
	"SELECT COUNT(*) \
		AS comment_count \
		FROM comments \
		WHERE post_id = ?";
	...
	let selected: Vec<Article> = pool.prep_exec(SELECT_ARTICLES, ())
		.map(|result| {
			...
			result.map(|x| x.unwrap()).map(|row| {
				let (id, title, content, post_time, edit_time, category) = mysql::from_row(row);
				// Get amount of comments on post
				let comment_count = mysql::from_row(
					pool.first_exec(SELECT_COMMENT_COUNT, (id,)).unwrap().unwrap()
				);
